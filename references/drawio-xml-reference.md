...
> **Source:** Adapted from `jgraph/drawio-mcp` at [`shared/xml-reference.md`](https://github.com/jgraph/drawio-mcp/blob/main/shared/xml-reference.md).
>
...
>
> Vendored here so the skill works offline and so `SKILL.md` can link to it
> without requiring a network fetch every run. Keep in lock-step with upstream;
> to refresh, run `scripts/download_reference_set.py` (or fetch the upstream
> file directly via `gh api`).
