...
Official draw.io documentation and examples can be cloned locally for deep
lookup. Run once from the skill root:

```bash
bash scripts/fetch_upstream_docs.sh
```

This populates `references/upstream/` (gitignored, ~30MB). Add
`--with-mxgraph` for the archived deep model docs, or `--with-app-templates`
for the built-in app templates (CC BY 4.0, attribution required).

If the clones are absent and network access is unavailable, fall back to the
vendored digests under `references/fetched/` and the local overlay files; do
not block a diagram request on this fetch.
...
Note: many files under `drawio-diagrams/templates/` store the model compressed
(base64 deflate inside `<diagram>`); prefer `blog/` and `examples/`, which are
mostly plain XML you can read directly.

## What the official examples actually do

Measured across 133 plain-XML official diagrams (2,956 edges) in
`drawio-diagrams`:
...
Official diagrams do not rely on fully automatic routing. Where a route
matters, they pin the connection side and add waypoints. Match that practice.
...
- Connector how-tos (waypoints, fixed vs floating, labels):
  https://www.drawio.com/docs/manual/connectors/
- Fixed vs floating connectors:
  https://www.drawio.com/docs/manual/connectors/connector-fixed-vs-floating/
- Custom connection points on a shape:
...
- mxConstants (all STYLE_*/EDGESTYLE_* keys):
...
- `jgraph/drawio` `src/main/webapp/templates/`: CC BY 4.0 (its own LICENSE
  file inside that directory). Attribute if any template content is reused.
